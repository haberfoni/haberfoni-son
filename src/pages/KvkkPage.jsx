...
const KvkkPage = () => {
    ...
    useEffect(() => {
        const fetchPage = async () => {
            try {
                const data = await adminService.getPageBySlug('kvkk');
                if (data) {
                    setPageData(data);
                }
            } catch (error) {
    ...
    return (
        <div className="container mx-auto px-4 py-12">
            <SEO
                title={pageData?.meta_title || "KVKK Aydınlatma Metni"}
                description={pageData?.meta_description || "Haberfoni Kişisel Verilerin Korunması Kanunu (KVKK) aydınlatma metni."}
                url="/kvkk"
            />
