...
const Hero = ({ items = [] }) => {
    ...
    // Track ad views - only run when items are loaded
    useEffect(() => {
        if (items.length === 0) return;

        const trackView = (adId) => {
            const viewedAds = JSON.parse(sessionStorage.getItem('viewed_headline_ads') || '[]');
            if (!viewedAds.includes(adId)) {
                adminService.incrementAdView(adId).catch(console.error);
                sessionStorage.setItem('viewed_headline_ads', JSON.stringify([...viewedAds, adId]));
            }
        };

        // Track views for all ads in the slider
        items.forEach(item => {
    ...
    }, [items.length]); // Trigger when items.length changes
                    ...
                    >
                        {items.map((item, index) => {
                            ...
                            if (isAd) {
                                ...
                                linkUrl = `/kategori/${slugify(item.category)}/${item.slug || slugify(item.title)}?from=slider`;
                            }
                            ...
                            const content = (
                                <>
                                    ...
                                    <div className="absolute bottom-0 left-0 p-6 w-full">
                                        {isAd && (
                                            <span className="inline-block px-2 py-1 bg-orange-500 text-white text-xs font-bold uppercase mb-2">
                                                🎯 REKLAM
                                            </span>
                                        )}
                                        {!isAd && (
                            ...
                            <>
                                ...
                                <div className="flex flex-col justify-between">
                                    ...
                                    <div className="flex items-center text-xs text-gray-500 mt-1">
                                        {isAd ? (
                                            <span className="text-orange-500 font-bold">🎯 REKLAM</span>
                                        ) : (
                                            <>
